...
class Client:
    def __init__(self, config, _endpoint="", _region_id="", _host_model="", _protocol="", _read_timeout=0,
                 _connect_timeout=0, _signature_version="", _addtional_headers=None, _local_addr="", _http_proxy="",
                 _https_proxy="", _no_proxy="", _user_agent="", _socks_5proxy="", _is_enable_crc=False,
                 _is_enable_md5=False, _socks_5net_work="", _max_idle_conns=0, _credential=None):
    ...
    def put_bucket_lifecycle(self, request, runtime):
        ...
        while TeaCore.allow_retry(_runtime["retry"], _retry_times, _now):
            ...
            try:
                ...
                _request.headers = {
                    "host": OSSUtilClient.get_host(request.bucket_name, self._region_id, self._endpoint,
                                                   self._host_model),
                ...
                _request.headers["authorization"] = OSSUtilClient.get_signature(_request, request.bucket_name,
        ...
        while TeaCore.allow_retry(_runtime["retry"], _retry_times, _now):
            ...
            try:
                ...
                if not UtilClient.empty(token):
                    _request.headers["x-oss-security-token"] = token
                _request.body = req_body
                if not UtilClient.is_unset(request.header.to_map()) and not UtilClient.empty(
                        request.header.content_md5):
        ...
        while TeaCore.allow_retry(_runtime["retry"], _retry_times, _now):
            ...
            try:
                ...
                if not UtilClient.empty(token):
                    _request.headers["x-oss-security-token"] = token
                _request.query = UtilClient.stringify_map_value(request.filter.to_map())
                _request.headers["authorization"] = OSSUtilClient.get_signature(_request, "", access_key_id,
                                                                                access_key_secret,
                                                                                self._signature_version,
                                                                                self._addtional_headers)
                _last_request = _request
    ...
    def append_object(self, request, runtime):
        ...
        while TeaCore.allow_retry(_runtime["retry"], _retry_times, _now):
            ...
            try:
                ...
                if self._is_enable_crc and not UtilClient.equal_string(ctx["crc"],
                                                                       _response.headers["x-oss-hash-crc64ecma"]):
                    raise TeaException({
                        "code": "CrcNotMatched",
                        "data": {
                            "clientCrc": ctx["crc"],
    ...
    def copy_object(self, request, runtime):
        ...
        while TeaCore.allow_retry(_runtime["retry"], _retry_times, _now):
            ...
            try:
                ...
                if not UtilClient.empty(token):
                    _request.headers["x-oss-security-token"] = token
                _request.headers["x-oss-copy-source"] = OSSUtilClient.encode(_request.headers["x-oss-copy-source"],
                                                                             "UrlEncode")
                _request.headers["authorization"] = OSSUtilClient.get_signature(_request, request.bucket_name,
                                                                                access_key_id, access_key_secret,
                                                                                self._signature_version,
